		...
		'2d', {
				init: function() {
						...
						function buzz(particle) {
								...
								particle.velocity.add(Vector.random(.25));
						...
						function stimulate(particle) {
								...
								if (particle.stimulated > 0) {
										particle.stimulated *= .95;
								}

								for (; i < l; i++) {

										if (particles[i] === particle || Vector.distanceSquared(particles[i].position, particle.position) > 50) {
												continue;
										}
				...
				paint: function(particle) {
						...
						var l = particle.life;
						var pulse = .5 + (.5 * (Math.abs(Math.sin(l * .15))));

						this.paint.circle(p.x, p.y, 1, 'rgba(255, 199, 0, 0)');
						this.paint.circle(p.x - v.y * 2, p.y - v.x * 2, s * particle.size * .15, 'rgba(255,50,0,' + (.5 * (s / 30) * pulse) + ')');
						this.paint.circle(p.x, p.y, .75 + (s * particle.size * .075), 'rgba(255,200,0,' + (.35 + ((s / 30) * pulse)) + ')');
